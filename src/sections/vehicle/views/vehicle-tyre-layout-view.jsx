...
export function VehicleTyreLayoutView({ vehicle }) {
    ...
    const [selectedLayoutId, setSelectedLayoutId] = useState(vehicle?.tyreLayoutId || null);

    useEffect(() => {
        if (vehicle?.tyreLayoutId) {
            setSelectedLayoutId(vehicle.tyreLayoutId);
        }
    }, [vehicle]);
    ...
    const handleSave = async () => {
        try {
            ...
            // Success handled by hook toast usually, but we can add extra logic if needed
        } catch (error) {
            console.error(error);
        }
    };
        ...
        <Stack spacing={3}>
            <Stack direction="row" alignItems="center" justifyContent="space-between">
                ...
                <Button
                    variant="contained"
                    onClick={handleSave}
                    disabled={!selectedLayoutId || selectedLayoutId === vehicle.tyreLayoutId}
                >
                    Save Layout
                </Button>
            </Stack>
